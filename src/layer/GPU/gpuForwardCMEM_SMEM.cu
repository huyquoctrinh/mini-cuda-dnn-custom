...
__host__ void GPUInterface::conv_forward_gpu_prolog(
    ...
    int inputSize  = batch_size * inputFeatureMapsQuantity * dimensionHeight * dimensionWeight * sizeof(float);  // input features map is inputFeatureMapsQuantity
    int outputSize = batch_size * outputFeatureMapsQuantity * H_out * W_out * sizeof(float); // output feature map is outputFeatureMapsQuantity
    int maskSize = outputFeatureMapsQuantity * inputFeatureMapsQuantity * kernelSize * kernelSize * sizeof(float); // inputFeatureMapsQuantity * outputFeatureMapsQuantity filter Maps of size K*K
...
__host__ void GPUInterface::conv_forward_gpu(
    ...
    // smem_size
    int smem_size = inputFeatureMapsQuantity * (TILE_WIDTH + kernelSize - 1) * (TILE_WIDTH + kernelSize - 1) * sizeof(float);
    ...
    //launch the kernel
    conv_forward_kernel<<<gridSize, blockSize, smem_size>>>(
        device_y,
        device_x,
        device_k,
        batch_size,
